...
class Rubrica {
  ...
  public void aggiungiSorted(Contatto cont) {
    if (cont == null) {
      return;
    }
    ...
    int middleIndex = numContatti / 2;
    Contatto middleCont = contatti[middleIndex];

    if (middleCont.getNome().compareTo(cont.getNome()) == 0) {
    }
  }

  public void raddoppiaContatti() {
    if (contatti == null) {
      return;
    }

    Contatto doubleContatti[] = new Contatto[contatti.length * 2];
    int i = 0;
